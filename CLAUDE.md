...
**Fact 1 — OIDC genuinely cannot move a dist-tag. VERIFIED against npm's docs, not inferred.** docs.npmjs.com/trusted-publishers states *"OIDC authentication supports the `npm publish` and `npm stage publish` commands"* and *"Other npm commands such as `install`, `view`, or `access` still require traditional authentication methods."* The trusted-publisher config says the same in its own field: `Allowed = npm publish`. So the alignment step **must** use token auth — there is no token-free path, and there will not be one until npm extends OIDC scope. (An earlier revision of this file called that explanation "wrong". It was not: it was right about *why* a token is needed, and merely silent about the bug in *how* the token was supplied.)
