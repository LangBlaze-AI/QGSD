...
round: 1
mode: "A"
verdict: Now I have a complete picture. Here's my analysis and surgical plan:

---

**Root Cause (two bugs, both in `sweepCtoR`):**

1. **`src/` not scanned** (line 2501): `scanDirs = ['bin', 'hooks']` — any orphan file injected into `src/` is invisible to C→R. The benchmark expects `src/*.js` to be detected.

2. **Stale index in `--fast` mode** (line 4670): `computeResidual()` only calls `rebuildCodeTraceIndex()` when `!effectiveFastMode()`. In fast mode, `loadCodeTraceIndex()` returns the cached/stale 
...
## Reasoning
Now I have a complete picture. Here's my analysis and surgical plan:
...
2. **Stale index in `--fast` mode** (line 4670): `computeResidual()` only calls `rebuildCodeTrace
