...
### Changed
- **`@next` is deprecated and scheduled for retirement — the `next == latest` invariant is retired with it.** `@next` was the prerelease channel; #366/#367 retired that flow, leaving a tag that by policy always equals `@latest`, so it carries **zero information** while costing a manual step after every release. Keeping it in sync cannot be automated: [npm's docs](https://docs.npmjs.com/trusted-publishers) scope OIDC to `npm publish` / `npm stage publish`, so `npm dist-tag` needs token auth — and per [GitHub's 2026-07-31 changelog](https://github.blog/changelog/2026-07-31-restricting-npm-bypass-2fa-granular-access-tokens/), 2FA-bypass tokens already cannot create tokens or change package access without an interactive challenge, and **from a targeted January 2027 lose direct publishing capability entirely**. Automating this would mean investing in a mechanism npm is retiring on a published timeline. Deprecating `@next` instead removes the **last token dependency** from the release pipeline — publish is already OIDC, and the dist-tag step was the only thing still needing a credential. Consequences in force now: drift is **expected, not an incident**; `@latest` is the only supported channel; alignment is opportunistic (manually, or via the `Align dist-tags` workflow) and never gates a release. `publish.yml`'s drift report is downgraded from `::warning::` to `::notice::DIST-TAG DRIFT (expected)` accordingly — an every-release warning that means nothing is how readers learn to ignore warnings, which is exactly what the false "unrecognized dispatch flag" alarm cost in #385. **Not removing the tag yet:** ~1.2k downloads/month means someone may be pinned to `@next`, and `npm dist-tag rm` would make `npm i @nforma.ai/nforma@next` fail outright. It stays pointing at a real version; revisit after two or three releases.
