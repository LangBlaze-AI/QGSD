...
### Added
- feat(gsd): **12 GSD-core capability ports, each fused with quorum or formal.** Ported the newer open-gsd/gsd-core planning/verification stack into nForma's GSD sector, reimplementing every backend natively (no `gsd-tools` coupling inherited) and fusing a differentiator rather than copying: **assumption-delta** → suggested TLA+/Alloy invariants; **doc-verifier** → a formal-artifact claim category + quorum escalation; **schema-gate** (plan:pre `[BLOCKING]` DB-push); **gap-analysis** (plan:post coverage, formally-modeled-but-unplanned = HIGH gap); **`/nf:autonomous`** — an unattended phase loop with quorum-gated planning and formal+harden-gated verification; **nyquist** validate-phase auditor (formal-priority gaps → harden/solve); **seeds** (forward-looking capture → close-formal-gaps); **extract-learnings + forensics**; **spec-phase** ambiguity gate (→ candidate invariants); the **eval pipeline** (AI-feature audit scored *through quorum*); **pattern-mapper** (→ formal contracts); and **next/smart-entry** (state-aware router). (#328–#341)
- feat(quorum): **CE-5 full convergence.** The deliberation loop no longer stops at first unanimity — it runs until a round is unanimous **and** proposes no new improvement **and** every improvement was reviewed by every voter (`quorum.full_convergence`, default true; set false for legacy behaviour). "Consensus" now means the team has nothing left to add. Live-validated on the real fleet, where the extra rounds surfaced a genuine nf-solve bug. (#345)
- feat(dogfood): **skill-invocation-lint** (`bin/skill-invocation-lint.cjs`, gated in `test:ci`) — statically resolves every `node <tool>.cjs` / `subagent_type=nf-*` / `@workflow` reference across all skills, catching the "ships green but dead-ends at runtime" class. All 77 skills verified intact. (#342, #343)
...
### Fixed
- fix(dogfood): **6 real skill-execution bugs found by running all 68 existing skills** against a synthetic sandbox — all repo-layout-vs-install-layout gaps that only surface from an installed `~/.claude/nf-bin/` context: `/nf:debug`'s dead context-packer primary path; `/nf:proximity`'s bare `node bin/*.cjs` (broke outside the repo root); `proximity-embed.mjs` never shipped to nf-bin; a partial-vendored `xstate` (no `package.json`) that crashed `state-candidates`/`validate-traces`; `/nf:sync-baselines`' defaults resolving to the wrong installed dir; and `/nf:audit-milestone` reading a `requirements_completed` field `summary-extract` never emitted. (#344)
- fix(solve): **degraded-convergence guard** — the aggregate residual summed each layer as `(residual >= 0 ? residual : 0)`, coercing a *skipped* layer (`-1`) to `0`, so a `total === 0` could not distinguish genuinely-clean from layers-didn't-run (false convergence). Now surfaces `degraded` + `unmeasured_layers`; residual numbers unchanged (benchmark-neutral). Found by the live CE-5 quorum. (#346)
- fix(ci): fixed the flaky `install-virgin` timeout (30s→120s, was `cancelledByParent` on slow runners) and repaired the `detect-secrets` baseline gate (a line-shift from an earlier test made it fail on every PR). (#332, #326, #327)
