...
### Changed
- **`@next` is deprecated and scheduled for retirement — the `next == latest` invariant is retired with it.** `@next` was the prerelease channel; #366/#367 retired that flow, leaving a tag that by policy always equals `@latest`, so it carries **zero information** while costing a manual step after every release. Keeping it in sync *is* automatable — `.github/workflows/align-dist-tags.yml` does it — but only on a credential class npm is retiring: [npm's docs](https://docs.npmjs.com/trusted-publishers) scope OIDC to `npm publish` / `npm stage publish`, so `npm dist-tag` needs token auth, and per [GitHub's 2026-07-31 changelog](https://github.blog/changelog/2026-07-31-restricting-npm-bypass-2fa-granular-access-tokens/) 2FA-bypass tokens already cannot create tokens or change package access without an interactive challenge, and **from a targeted January 2027 they lose direct publishing capability entirely**. Automating this would mean investing in a mechanism npm is retiring on a published timeline. Deprecating `@next` instead removes the **last token dependency** from the release pipeline — publish is already OIDC, and the dist-tag step was the only thing still needing a credential. Consequences in force now: drift is **expected, not an incident**; `@latest` is the only supported channel; alignment is opportunistic (manually, or via the `Align dist-tags` workflow) and never gates a release. `publish.yml`'s drift report is downgraded from `::warning::` to `::notice::DIST-TAG DRIFT (expected)` accordingly — an every-release warning that means nothing is how readers learn to ignore warnings, which is exactly what the false "unrecognized dispatch flag" alarm cost in #385. **Not removing the tag yet:** ~1.2k downloads/month means someone may be pinned to `@next`, and `npm dist-tag rm @nforma.ai/nforma next` would make `npm i @nforma.ai/nforma@next` fail outright. It stays pointing at a real version; revisit after two or three releases.
- fix(quorum): **the stall timer no longer uses byte count as evidence of liveness (STALL-TIMEOUT-03).** `<500 bytes → apply a tighter kill window` was a proxy for "this slot is hung", and a live multi-model quorum falsified it the hard way — by killing three of its own five voters mid-answer. `claude-1` (idle 90s → 90s window), `copilot-1` (idle 30s → 30s), and `claude-z-ai` (90s) were all producing correct output when the timer fired; their partial answers were recoverable from `unavail_message`. `codex-1` and `antigravity-1` survived not because they were faster but because they **stream**, so every chunk rearmed the timer. The discriminator was never byte count. Worse, deriving the window from `idle_timeout_ms` (the #385 fix) made that field do double duty as "how long may this model think before output" — and for most slots the configured value (30s/90s) is far *smaller* than real think time, while the caller had budgeted 300s. There are now two windows, one per failure mode: **before the first byte**, `ttfb_timeout_ms` (default 30s) — zero output is the only honest evidence of a hang, so fast-fail belongs there; **after the first byte**, the inter-chunk ceiling — `inter_chunk_ceiling_ms` if the slot declares one, else the caller's idle budget. `stall_timeout_ms` is kept as a legacy alias for the ceiling, since the values already in that field were measured against post-preamble silence, so no config surface is added. Live-verified: `claude-1` answers in 134s where it previously died at 90s. Measuring also corrected an assumption — the `claude` CLI **buffers** (zero bytes until well past 30s) rather than emitting a preamble like the z-ai/minimax/kimi routes, which is why TTFB is per-slot configurable and why the claude-family slots carry `ttfb_timeout_ms: 150000`. Ratified by quorum (3/3 unanimous, Round 1: codex-1, antigravity-1, claude-minimax) over the alternative of keeping #386 as shipped; debate at `.planning/quorum/debates/2026-08-07-stall-timer-design-decision-a-vs-b.md`.

### Added
- feat(quorum): **deep-probe gate — preflight can now tell a quota-dead slot from a healthy one (P1 of #293, the last piece).** L1 (`--version`) proves a binary exists and L2 (`/models`) treats 401/403 as *reachable*, so neither notices a slot that is authenticated, reachable, and out of quota. Demonstrated live on 2026-08-08: preflight reported **7/7 available** while `claude-z-ai` (429, weekly limit) and `antigravity-1` (quota, 74h to reset) could not answer at all — a panel promising full quorum that then produces two UNAVAILs mid-round. `deepProbeSlot()` runs the slot's configured `deep_probe` and is deliberately **lopsided**: it downgrades *only* on a fast, explicit auth/quota signal in the slot's own output; a timeout, spawn error, or unrecognised response is `INCONCLUSIVE` and never downgrades. That asymmetry is the point — the opposite error, killing a slow-but-alive slot, is the one this codebase has already paid for three times (STALL-TIMEOUT-01..03). It runs only when the panel is already **degraded** *and* an explicit `--budget-ms` covers it (a naive auto-enable previously blew the `<8s` liveness budget with `spawnSync ETIMEDOUT`), or on demand via `--deep-probe`. Live-verified end to end: both dead slots are now downgraded with their real reasons and the gate reports `DEGRADED — 5 available`, where the cheap budget-less path still finishes in ~3s having skipped the probe entirely.
- feat(quorum): **preflight now validates the roster and decides the quorum gate (P3 + P5 of #293, extracted fresh).** `validateProviders()` reports config faults that would otherwise surface mid-quorum as an opaque UNAVAIL — the two this repo has actually shipped are a slot with neither `cli` nor `mainTool` (spawn receives `null`, taking the whole quorum offline, #197) and an `args_template` with no `{prompt}` placeholder (the CLI runs with flags and no question). Also errors on duplicate names (silently last-wins everywhere downstream), missing/unknown `type`, and http slots without a base URL or any auth; warns on non-positive timeout fields and on third-party-routed slots with no `deep_probe`. Non-fatal by design: it reports on every `--all` and never blocks the preflight. `computeQuorumGate()` turns *"available < max_quorum_size → BLOCK unless `--force-quorum`"* from prose in `commands/nf/quorum.md` into a machine field — `{available_count, quorum_met, degraded, blocked, waiver_required, waiver_used, gate_reason}` — because a gate that depends on a model reading a paragraph correctly is not a gate. It keeps **degraded** and **blocked** distinct (below `max_quorum_size` but above `min_live_voters` still dispatches), and a `--force-quorum` waiver unblocks while being *recorded* rather than silently making the panel look healthy. 11 tests, each of the three core behaviours mutation-proven.
- feat(release): **`Align dist-tags` — a manually dispatchable workflow that repairs `@next` drift without a one-time password.** `publish.yml` aligns `@next` automatically after every publish since #391, but that cannot help a release published *before* that fix, or an alignment that failed transiently. Doing it by hand needs `npm dist-tag add`, which on a 2FA-protected account fails with `EOTP` and demands an authenticator code — so it cannot be scripted, and in practice it simply does not get done (0.44.3 shipped with `next` still on 0.44.2). The `NPM_TOKEN` secret is 2FA-exempt, which is what automation tokens are for, so CI can do unattended what an interactive human cannot. Scope is deliberately narrow: it moves a dist-tag and cannot publish. It refuses a prerelease target (a prerelease holding `@next` would silently install for everyone under the alias policy), and — unlike `publish.yml`'s verify step, which must never fail a job whose package is already out — this one **fails** if the tag did not move, because a green manual run that changed nothing is the exact outcome worth preventing. Each `run:` block was executed against a stubbed `npm view` across all four branches (aligned / not-moved / unreadable / prerelease) rather than reviewed by eye.
- feat(quorum): **timeout records now say which config produced the window (I5).** `resolveTtfbTimeout` / `resolveInterChunkCeiling` each compute a `source` (`per-slot` / `default` / `idle-budget` / `legacy-stall`) and `recordTelemetry` was dropping both it and the resolved values, so a TIMEOUT in `quorum-rounds-*.jsonl` could be seen but never attributed — the number alone never answers "did *my* config do this?", since today's per-slot value can be tomorrow's default. Records now carry `ttfb_timeout_ms`, `ttfb_source`, `inter_chunk_ceiling_ms` and `inter_chunk_source`, always present (null when unknown) so the schema stays stable for analysis. Passed as a trailing options object rather than a 15th positional parameter. 4 tests drive the **real child** and read the actual log, so the fields are proven to survive the whole dispatch path; mutation-proven. Accepted as improvement I5 by the 2026-08-07 quorum.

### Fixed
- fix(quorum): **the shared error classifier missed `"quota reached"`** — it only knew `"quota exceeded"`, so antigravity's `"Individual quota reached. Resets in 74h38m39s"` was **not classified at all** and the slot was reported healthy with three days of cooldown left. Added `quota reached`, `limit exhausted` and `limit reached`, checked against real messages and against ordinary prose a model might emit (`"I reached the conclusion that the limit is fine"` must not match).
- fix(provider-status): `main()` ran unconditionally on `require()`, so importing the module for its classifier would execute the entire probe sweep — the #198 side-effect-on-require class. Guarded with `require.main === module` and the classifier is now exported, so `quorum-preflight` shares one pattern list instead of forking the definition that decides whether a slot is declared dead.
- fix(quorum): **a quota-dead slot is now held out until it actually resets, instead of being retried every 5 minutes for days (QUOTA-RESET-01).** Providers state their reset window, but in shapes `parseAvailabilityHint` did not match — `antigravity-1` says `"Resets in 95h26m37s."` and `claude-z-ai` says `"...will reset at 2026-08-09 10:01:44"`, and neither the `"in N hours"` nor the `"until <Month Day>"` pattern covers them. Both fell through to the 5-minute default, so a slot that was dead for **four days** burned a dispatch in every quorum round, dragging the live-voter count toward the `min_live_voters` floor. Two new shapes are parsed: a compact duration (`95h26m37s`, `32h54m`, `90s`) and an absolute `reset at YYYY-MM-DD HH:MM:SS`. Deliberately conservative in the dangerous direction — a plain `STALL`/`IDLE_TIMEOUT` still gets the 5-minute default, because over-matching would retire a healthy slot for days on one bad round. The compact parser is written as locate-token-then-decompose: a single pattern of all-optional unit groups matches the empty string, "succeeding" with every capture undefined and silently falling through to the very default it exists to replace (which is what the first version did). 6 tests, mutation-proven in both directions. This is P4 of the long-open #293, extracted fresh.
- fix(install): **a stale backup no longer clobbers live slot config on every install.** When `bin/providers.json` ships empty (it always does), install reconstructs the user's `providers.json` from `~/.claude.json` and overlays what it can recover for each slot, gathering from four candidates documented as being "in order of freshness" — live `nf-bin/` first, then two `nf-local-patches/` backups, then the legacy live path. The gather loop called `map.set(name, entry)` unconditionally, so the **last** candidate won: the exact inversion of the stated order. On the machine that surfaced it, a `nf-local-patches/nf/bin/providers.json` dated **2026-05-13** overrode the live entry for 7 slots on every single install, silently reverting per-slot tuning (`ttfb_timeout_ms`, `inter_chunk_ceiling_ms`, `idle_timeout_ms`, `model`, `env`). It was found because freshly-set `ttfb_timeout_ms` values kept vanishing after `install.js --claude --global`, which also explains earlier "mystery" resets of `stall_timeout_ms`. First writer now wins, so backups do what they exist for — recovering slots the live file has **lost** — and never override it. 4 tests in `bin/install-providers-precedence.test.cjs` execute the extracted gather loop against stubbed candidates; mutation-proven, and verified against a real install that now preserves every tuned field.

## [0.44.3] - 2026-08-07 — Quorum stall-window derivation, no shipped user config, and migration/portability guards

### Fixed
- fix(install): **`--migrate-slots` no longer renames a live Daintree preset slot.** `SLOT_MIGRATION_MAP` treats `claude-minimax` / `claude-kimi` / `claude-glm` as legacy *model*-based names, but that is also exactly the shape `/nf:link-daintree` produces today (`{agentName}-{slug(preset.name)}`) — so a working preset clone could carry a name the map wanted to rename. The rename moved the `mcpServers` key to `claude-2` while `providers.json` still said `claude-minimax`, orphaning the provider entry and deleting the `mcp__claude-minimax__…` tool the quorum dispatches to. Preset clones are identifiable (link-daintree stamps `daintree_preset_id`) and are now skipped, with the skip reported rather than silently folded into "already migrated". Two further hardenings on the same path: a rename that *does* apply now updates `providers.json` in lockstep instead of leaving a dangling pair, and provider resolution goes through `resolve-providers.cjs` (issue #197) rather than an inline path. Fails open when `providers.json` is absent or corrupt — the pre-slot install this migration exists for has none. 4 tests, mutation-proven red in both directions (guard removed → a preset slot is renamed; lockstep removed → the pair dangles).
- fix(scripts): **`scripts/tui-*.tape` no longer hardcode the maintainer's home directory.** Both shipped to npm containing `cd /Users/jonathanborduas/code/QGSD`, a line nobody else can execute. They now use `cd ${NF_REPO:-$PWD}` (verified against real `vhs`: with `NF_REPO` set the `cd` takes effect, and unset it falls back to the shell's cwd, which vhs sets to its own), `npm run assets:tui` exports `NF_REPO="$PWD"`, and `scripts/*.tape` drops out of `package.json` `files` — asset-generation tooling is not part of what users install, the same reasoning that already excludes `scripts/generate-*-svg.js`. New `lint-isolation` Rule 8 (`machine-specific-path`) gates the class: no `/Users/<name>/` or `/home/<name>/` path in a file that is *executed as written* (`.tape`, `.sh`, `.yml`/`.yaml` under `scripts/` and `.github/workflows/`) — the same defect that once broke `benchmark-sync.yml` in CI. Scoped to those formats so illustrative paths in JS comments stay legal, with comment lines and obvious placeholders (`/Users/foo/`, `/home/<name>/`, `/home/runner/`) exempt. 5 tests in `bin/machine-path-lint.test.cjs`, including one asserting the rule is actually *invoked* and not merely defined; mutation-proven red.
