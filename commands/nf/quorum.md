...
```bash
NF_RESULT_FILE=".planning/quorum/slot-results/<slot>-round-<N>.txt" node -e 'const {validateSlotResult}=require(process.env.HOME+"/.claude/nf-bin/quorum-slot-dispatch.cjs");const fs=require("fs");const f=process.env.NF_RESULT_FILE;const c=fs.existsSync(f)?fs.readFileSync(f,"utf8"):"";const r=validateSlotResult(c);console.log(JSON.stringify(r));process.exit(r.genuine?0:1)'
```
