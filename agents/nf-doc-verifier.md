...
Spawned by the `/nf:verify-docs` workflow. Each spawn receives a `<verify_assignment>` XML block containing:
...
**6. Formal-artifact claims (nForma)**
Docs in a nForma project often assert things about the formal layer. Verify these against `.planning/formal/`:
- **Requirement IDs** — backtick tokens matching `[A-Z][A-Z0-9]+-[0-9]+` (e.g., `WIZ-05`, `ADAPT-01`). Verification: check the ID appears in `.planning/formal/requirements.json` (JSON-aware lookup) or `.planning/REQUIREMENTS.md`. FAIL with `{ ..., expected: "requirement <ID> in requirements.json", actual: "no such requirement" }` if absent.
- **Model names** — references to a `.tla`/`.als`/`.pnml` model (e.g., "the `QGSDSetupWizard` TLA+ model"). Verification: Glob `.planning/formal/**/{name}.{tla,als,pnml}`. FAIL if the named model does not exist.
- **Invariant claims** — a doc that says an invariant/property holds (e.g., "the model proves `ConfirmBeforeDone`"). Verification: Grep the named invariant in `.planning/formal/**/*.{tla,als}`. PASS if the invariant is defined; mark **UNVERIFIABLE (WARNING)** if the doc claims it *holds* but you can only confirm it is *defined* (a static grep cannot prove a TLC/Alloy result) — never assert a proof result from the filesystem alone.
Fail-open: if `.planning/formal/` does not exist, SKIP this category (not a nForma-formal project).
</claim_extraction>
...
- **GSD marker**: The comment `<!-- generated-by: nf-doc-writer -->` — skip entirely.
