...
Read Current Test section from UAT file.
    ...
    if grep -q '"playwright"' "$DISCOVER_JSON" && grep -q '\.spec\.' "$DISCOVER_JSON"; then
      ...
      NF_DISCOVER="$DISCOVER_JSON" NF_BATCH="$BATCH_FILE" node -e "const fs=require('fs'); let d; try{ d=JSON.parse(fs.readFileSync(process.env.NF_DISCOVER,'utf8')); }catch(e){ process.stderr.write('verify-work: test-discovery JSON malformed ('+e.message+') — skipping automated batch\n'); process.exit(0); } const files=(d.by_runner&&d.by_runner.playwright)||d.test_files||[]; if(!Array.isArray(files)||files.length===0) process.exit(0); const m={batch_id:Date.now(), runner:'playwright', files:files}; fs.writeFileSync(process.env.NF_BATCH, JSON.stringify(m));"
    else
      # Try jest files as fallback
      NF_DISCOVER="$DISCOVER_JSON" NF_BATCH="$BATCH_FILE" node -e "const fs=require('fs'); let d; try{ d=JSON.parse(fs.readFileSync(process.env.NF_DISCOVER,'utf8')); }catch(e){ process.stderr.write('verify-work: test-discovery JSON malformed ('+e.message+') — skipping automated batch\n'); process.exit(0); } const files=(d.by_runner&&d.by_runner.jest)||d.test_files||[]; if(!Array.isArray(files)||files.length===0) process.exit(0); const m={batch_id:Date.now(), runner:'jest', files:files}; fs.writeFileSync(process.env.NF_BATCH, JSON.stringify(m));"
    fi
