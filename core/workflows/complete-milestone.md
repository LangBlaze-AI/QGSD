...
2. **Run audit first** — `/nf:audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ~/.claude/nf/bin/nf-tools.cjs summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node ~/.claude/nf/bin/nf-tools.cjs milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/nf:cleanup` later to archive retroactively.
...
- [ ] User knows next step (/nf:new-milestone)
