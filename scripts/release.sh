...
echo ""
echo "  1. publish.yml  → Tests + publishes to npm via OIDC (no NPM_TOKEN)"
echo "                    prerelease tag → @next; stable main push → @latest + tag + GitHub Release"
echo ""
echo "IMPORTANT: For a stable release (no -rc suffix), publish.yml BEST-EFFORT aligns"
echo "@next to match @latest. If OIDC can't authorize the dist-tag it warns; align with:"
echo "  npm dist-tag add @nforma.ai/nforma@<version> next"
echo "A prerelease publishes to @next only."
echo ""
echo "Verify dist-tags after publish:"
